...
static constexpr std::wstring_view SYSTEM_VALUE_NAME = L"SystemUsesLightTheme";
  ...
  static constexpr WORD DEFAULT_CONSOLE_COLOR = FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE;
    ...
    bool GetRegistryValue(HKEY hKey, std::wstring_view valueName, DWORD& outValue) {
  ...
  LONG result = RegSetValueExW(hKey, valueName.data(), 0, REG_DWORD,
   ...
   // Uses SMTO_ABORTIFHUNG to prevent blocking on hung windows
        // 50ms timeout is enough for Explorer to receive without noticeable delay
        DWORD_PTR result;
 ...
 50,  // 50ms - fast enough to feel instant, reliable enough for Explorer
     &result
        );
   ...
   &result
        ...
        // Always return true since we're fire-and-forget
        return true;
    }
...
public:
 ...
 RegKey keyRead;
  ...
  throw std::runtime_error("Failed to open registry key - theme settings may not exist");
        ...
        DWORD currSystem = 0;
 DWORD currApps = 0;
        bool hasSystem = GetRegistryValue(keyRead, SYSTEM_VALUE_NAME, currSystem);
        bool hasApps = GetRegistryValue(keyRead, APPS_VALUE_NAME, currApps);

        // Close read-only handle immediately
  keyRead.Close();

        // If values don't exist, default to dark (0)
